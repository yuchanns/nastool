...
from app.db.models import MEDIASYNCITEMS, MEDIASYNCSTATISTIC, BaseMedia
...
class MediaDb:
    ...
    def exists(self, server_type, title, year, tmdbid):
        ...
        if tmdbid:
            count = (
                ...
                .filter(MEDIASYNCITEMS.TMDBID == str(tmdbid))
                .count()
            )
            if count:
                return True
    ...
    def get_statistics(self, server_type):
        if not server_type:
            return None
        return (
            self.session.query(MEDIASYNCSTATISTIC)
            .filter(MEDIASYNCSTATISTIC.SERVER == server_type)
            .first()
        )
