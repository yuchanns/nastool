...
from config import (
    ...
    PT_TRANSFER_INTERVAL,
    REFRESH_PT_DATA_INTERVAL,
...
class Scheduler:
    ...
    def run_service(self):
        ...
        if self._pt:
            ...
            if ptsignin_cron:
                if "-" in ptsignin_cron:
                    ...
                    try:
                        ...
                        hours = 0
                    if hours:
                        self.SCHEDULER.add_job(Sites().signin, "interval", hours=hours)
                        log.info("站点自动签到服务启动")
            ...
            if pt_monitor:
                self.SCHEDULER.add_job(
                    Downloader().transfer, "interval", seconds=PT_TRANSFER_INTERVAL
                )
                log.info("下载文件转移服务启动")
