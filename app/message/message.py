...
class Message(object):
    ...
    def __init__(self):
        self._message_schemas = SubmoduleHelper.import_submodules(
            "app.message.client", filter_func=lambda _, obj: hasattr(obj, "schema")
        )
    ...
    def init_config(self):
        ...
        for client_config in self.dbhelper.get_message_client() or []:
            ...
            if not client_config.ENABLED or not config:
                continue
            client = {
                "search_type": ModuleConf.MESSAGE_CONF.get("client")
    ...
    def get_status(self, ctype=None, config=None):
        ...
        state, ret_msg = self.__build_class(ctype=ctype, conf=config).send_msg(
            title="测试", text="这是一条测试消息", url="https://github.com/jxxghp/nas-tools"
        )
        if not state:
    ...
    def __sendmsg(self, client, title, text="", image="", url="", user_id=""):
        ...
        state, ret_msg = client.get("client").send_msg(
            title=title, text=text, image=image, url=url, user_id=user_id
        )
        if not state:
    ...
    def send_channel_msg(self, channel, title, text="", image="", url="", user_id=""):
        ...
        if client:
            state = self.__sendmsg(
                client=client, title=title, text=text, image=image, url=url, user_id=user_id
            )
            return state
        return False
    ...
    def send_transfer_movie_message(self, in_from: Enum, media_info, exist_filenum, category_flag):
        ...
        msg_str = (
            f"{msg_str}，大小：{StringUtils.str_filesize(media_info.size)}，来自：{in_from.value}"
        )
        if exist_filenum != 0:
    ...
    def send_download_fail_message(self, item, error_msg):
        ...
        for client in self._active_clients:
            if "download_fail" in client.get("switchs"):
                self.__sendmsg(
                    client=client, title=title, text=text, image=item.get_message_image()
                )
    ...
    def send_transfer_fail_message(self, path, count, text):
        ...
        for client in self._active_clients:
            if "transfer_fail" in client.get("switchs"):
                self.__sendmsg(client=client, title=title, text=text, url="unidentification")
    ...
    def send_brushtask_remove_message(self, title, text):
        ...
        for client in self._active_clients:
            if "brushtask_remove" in client.get("switchs"):
                self.__sendmsg(client=client, title=title, text=text, url="brushtask")
    ...
    def send_mediaserver_message(self, title, text, image):
        ...
        for client in self._active_clients:
            if "mediaserver_message" in client.get("switchs"):
                self.__sendmsg(client=client, title=title, text=text, image=image)
