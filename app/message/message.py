...
class Message(object):
    ...
    def init_config(self):
        ...
        for client_config in self.dbhelper.get_message_client() or []:
            ...
            client_conf = {
                ...
                "config": config,
                "switchs": json.loads(client_config.SWITCHS)
    ...
    def get_status(self, ctype=None, config=None):
        ...
        state, ret_msg = self.__build_class(ctype=ctype, conf=config).send_msg(
            title="测试",
            text="这是一条测试消息",
            url="https://github.com/yuchanns/nastool",
        )
        if not state:
    ...
    def send_channel_msg(self, channel, title, text="", image="", url="", user_id=""):
        ...
        self.messagecenter.insert_system_message(
            level="INFO", title=title, content=text
        )
        # 发送消息
    ...
    def send_download_message(self, in_from: SearchType, can_item):
        ...
        # 插入消息中心
        self.messagecenter.insert_system_message(
            level="INFO", title=msg_title, content=msg_text
        )
        # 发送消息
    ...
    def send_transfer_movie_message(
        ...
        # 插入消息中心
        self.messagecenter.insert_system_message(
            level="INFO", title=msg_title, content=msg_str
        )
        # 发送消息
        for client in self._active_clients:
    ...
    def send_transfer_fail_message(self, path, count, text):
        ...
        for client in self._active_clients:
            if "transfer_fail" in client.get("switchs"):
                self.__sendmsg(
                    client=client, title=title, text=text, url="unidentification"
                )
