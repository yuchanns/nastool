...
class DoubanSync:
    ...
    def init_config(self):
        ...
        if douban:
            # 同步间隔
            self._interval = (
                int(douban.get("interval"))
                if str(douban.get("interval")).isdigit()
                else None
            )
            ...
            # 时间范围
            self._days = (
                int(douban.get("days")) if str(douban.get("days")).isdigit() else None
            )
            # 类型
    ...
    def sync(self):
        ...
        with lock:
            ...
            for media in medias:
                ...
                try:
                    # 查询数据库状态，已经加入RSS的不处理
                    search_state = self.dbhelper.get_douban_search_state(
                        media.get_name(), media.year
                    )
                    if not search_state or search_state[0] == "NEW":
                        if self._auto_search:
                            ...
                            if not media_info or not media_info.tmdb_info:
                                log.warn(
                                    "【Douban】%s 未查询到媒体信息" % media.get_name()
                                )
                                continue
                            # 检查是否存在，电视剧返回不存在的集清单
                            ...
                            if exist_flag:
                                ...
                                self.dbhelper.insert_douban_media_state(
                                    media, "DOWNLOADED"
                                )
                                continue
                            if not self._auto_rss:
                                ...
                                code, msg, _ = self.subscribe.add_rss_subscribe(
                                    mtype=media.type,
                                    name=media.get_name(),
                                    year=media.year,
                                ...
                                if code != 0:
                                    ...
                                    self.message.send_rss_success_message(
                                        in_from=SearchType.DB, media_info=media
                                    )
                                    # 插入为已RSS状态
                                    self.dbhelper.insert_douban_media_state(
                                        media, "RSS"
                        ...
                        log.info(
                            "【Douban】%s %s 已处理过" % (media.get_name(), media.year)
                        )
                except Exception as err:
                    log.error(
                        "【Douban】%s %s 处理失败：%s"
                        % (media.get_name(), media.year, str(err))
                    )
                    continue
    ...
    def __get_all_douban_movies(self):
        ...
        """
        if not self._interval or not self._days or not self._users or not self._types:
        ...
        for user in self._users:
            ...
            for mtype in self._types:
                ...
                while True:
                    ...
                    try:
                        items = self.douban.get_douban_wish(
                            dtype=mtype, userid=user, start=start_number, wait=True
                        )
                        if not items:
                        ...
                        for item in items:
                            ...
                            if not date:
                                ...
                                if not (datetime.datetime.now() - mark_date).days < int(
                                    self._days
                                ):
                                    continue_next_page = False
                                    break
                        ...
                        log.error(
                            f"【Douban】{user_name or user} 第 {page_number} 页解析出错：%s"
                            % str(err)
                        )
                        break
                    # 继续下一页
                ...
                # 当前类型解析结束
                log.debug(
                    f"【Douban】用户 {user_name or user} 的 {mtype} 解析完成，共获取到 {user_type_succnum} 个媒体"
                )
            log.info(
                f"【Douban】用户 {user_name or user} 解析完成，共获取到 {user_succnum} 个媒体"
            )
        ...
        for doubanid, info in douban_ids.items():
            ...
            if not douban_info:
                log.warn(
                    "【Douban】%s 未正确获取豆瓣详细信息，尝试使用网页获取" % doubanid
                )
                ...
                if not douban_info:
                    ...
                    continue
            media_type = (
                MediaType.TV if douban_info.get("episodes_count") else MediaType.MOVIE
            )
            log.info(
                "【Douban】%s：%s %s".strip()
                % (media_type.value, douban_info.get("title"), douban_info.get("year"))
            )
            meta_info = MetaInfo(
                title="%s %s"
                % (douban_info.get("title"), douban_info.get("year") or "")
            )
