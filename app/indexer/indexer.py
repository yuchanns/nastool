...
class Indexer(object):
    ...
    def __init__(self):
        self._indexer_schemas = SubmoduleHelper.import_submodules(
            "app.indexer.client", filter_func=lambda _, obj: hasattr(obj, "schema")
        )
    ...
    def get_indexer_dict(self):
        ...
        获取索引器字典
        """
        return [{"id": index.id, "name": index.name} for index in self.get_indexers()]
    ...
    def search_by_keyword(
        ...
        if filter_args and filter_args.get("site"):
            log.info(
                f"【{self._client_type.value}】开始检索 %s，站点：%s ..."
            ...
            log.info(
                f"【{self._client_type.value}】开始并行检索 %s，线程数：%s ..."
        ...
        log.info(
            f"【{self._client_type.value}】所有站点检索完成，有效资源数：%s，总耗时 %s 秒"
