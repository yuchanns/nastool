...
class _IIndexClient(metaclass=ABCMeta):
    ...
    def filter_search_results(
        ...
        for item in result_array:
            ...
            uploadvolumefactor = (
                round(float(item.get("uploadvolumefactor")), 1)
                if item.get("uploadvolumefactor") is not None
                else 1.0
            )
            ...
            if (
                filter_args.get("seeders")
                and not indexer.public
                and str(seeders) == "0"
            ):
            ...
            if (
                ...
                and filter_args.get("type") == MediaType.MOVIE
            ):
                log.info(
            ...
            if not match_media:
                ...
                if (
                    ...
                    if match_media and str(cache_info.get("id")) == str(
                        match_media.tmdb_id
                    ):
                        # 缓存匹配，合并媒体数据
                        media_info = self.media.merge_media_info(meta_info, match_media)
                        ...
                        if not media_info:
                            log.warn(
                                f"【{self.index_type}】{torrent_name} 识别媒体信息出错！"
                            )
                            index_error += 1
                            continue
                        ...
                        if str(media_info.tmdb_id) != str(match_media.tmdb_id):
                            ...
                            continue
                        # 合并媒体数据
                        media_info = self.media.merge_media_info(
                            media_info, match_media
                        )
                # 过滤类型
                if filter_args.get("type"):
                    if (
                        filter_args.get("type") == MediaType.TV
                        and media_info.type == MediaType.MOVIE
                    ) or (
                        filter_args.get("type") == MediaType.MOVIE
                        and media_info.type == MediaType.TV
                    ):
                        log.info(
                ...
                if match_media.over_edition:
                    ...
                    if (
                        ...
                        continue
                    # 检查优先级是否更好
                    if match_media.res_order and int(res_order) <= int(
                        match_media.res_order
                    ):
                        log.info(
                            f"【{self.index_type}】{media_info.get_title_string()}{media_info.get_season_string()} "
                        ...
                        continue
            # 检查标题是否匹配季、集、年
            if not self.filter.is_torrent_match_sey(
                media_info,
