    ...
    def search(self, keyword, indexer, page=None, mtype=None):
        ...
        if method == "chrome":
            ...
            if params.get("referer"):
                referer = indexer.domain + params.get("referer").replace(
            ...
            if not search_input or not search_button:
                return []
            # 使用浏览器打开页面
            if not chrome.visit(url=search_url, cookie=indexer.cookie, ua=indexer.ua):
                return []
            ...
            try:
                ...
                # 等待可点击
                submit_obj = WebDriverWait(driver=chrome.browser, timeout=10).until(
                    es.element_to_be_clickable((By.XPATH, search_button))
                )
                if submit_obj:
                    # 输入用户名
        ...
        # 设置抓虫参数
        self.torrentspider.setparam(
            keyword=keyword, indexer=indexer, referer=referer, page=page, mtype=mtype
        )
        # 种子筛选器
        ...
        for torn in html_doc(torrents_selector):
            self.torrents_info_array.append(
                copy.deepcopy(self.torrentspider.Getinfo(PyQuery(torn)))
            )
