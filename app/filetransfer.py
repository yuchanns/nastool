...
from app.media import Category, Media, Scraper
from app.media.meta import MetaInfo
from app.mediaserver import MediaServer
from app.message import Message
from app.subtitle import Subtitle
from app.utils import EpisodeFormat, ExceptionUtils, PathUtils, StringUtils, SystemUtils
...
class FileTransfer:
        ...
        if len(file_list) == 0:
            ...
            for file_item in file_list:
                sub_file_name = re.sub(
                    _zhtw_sub_re,
                    ".",
                    re.sub(_zhcn_sub_re, ".", os.path.basename(file_item), flags=re.I),
                    flags=re.I,
                )
                ...
                if (
                    (
                        ...
                        == os.path.splitext(sub_file_name)[0]
                    )
                    or (
                        sub_metainfo.cn_name
                        and sub_metainfo.cn_name == metainfo.cn_name
                    )
                    or (
                        sub_metainfo.en_name
                        and sub_metainfo.en_name == metainfo.en_name
                    )
                ):
                    if (
                        metainfo.get_season_string()
                        and metainfo.get_season_string()
                        != sub_metainfo.get_season_string()
                    ):
                        continue
                    if (
                        metainfo.get_episode_string()
                        and metainfo.get_episode_string()
                        != sub_metainfo.get_episode_string()
                    ):
                        continue
                    new_file_type = ""
                    ...
                    new_sub_tag_list = [
                        (
                            ...
                            % (
                                new_file_type,
                                new_sub_tag_dict.get(new_file_type, ""),
                                t,
                            )
                        )
                        for t in range(6)
                    ...
                    for new_sub_tag in new_sub_tag_list:
                        new_file = (
                            os.path.splitext(new_name)[0] + new_sub_tag + file_ext
                        )
                        ...
                        try:
                            if not os.path.exists(new_file):
                                ...
                                if retcode == 0:
                                    log.info(
                                        "【Rmt】字幕 %s %s完成"
                                        % (os.path.basename(file_item), rmt_mode.value)
                                    )
                                    break
                                else:
                                    log.error(
    ...
    def __transfer_bluray_dir(self, file_path, new_path, rmt_mode):
        ...
        # 复制
        retcode = self.__transfer_dir_files(
            src_dir=file_path, target_dir=new_path, rmt_mode=rmt_mode, bludir=True
        )
        if retcode == 0:
    ...
    def __transfer_dir_files(self, src_dir, target_dir, rmt_mode, bludir=False):
        ...
        for file in file_list:
            ...
            if not os.path.exists(new_dir):
                os.makedirs(new_dir)
            retcode = self.__transfer_command(
                file_item=file, target_file=new_file, rmt_mode=rmt_mode
            )
            if retcode != 0:
                break
            else:
    ...
    def __transfer_origin_file(self, file_item, target_dir, rmt_mode):
        ...
        if os.path.isdir(file_item):
            log.info(
                "【Rmt】正在%s目录：%s 到 %s" % (rmt_mode.value, file_item, target_dir)
            )
            retcode = self.__transfer_dir_files(
                src_dir=file_item, target_dir=target_dir, rmt_mode=rmt_mode
            )
        # 文件
        else:
            ...
            if os.path.exists(target_file):
                ...
                return 0
            retcode = self.__transfer_command(
                file_item=file_item, target_file=target_file, rmt_mode=rmt_mode
            )
            if retcode == 0:
        ...
        if retcode == 0:
            ...
            log.error(
                "【Rmt】%s %s到unknown失败，错误码 %s"
                % (file_item, rmt_mode.value, retcode)
            )
        return retcode

    def __transfer_file(
        self, file_item, new_file, rmt_mode, over_flag=False, old_file=None
    ):
        """
        转移一个文件，同时处理字幕
        :param file_item: 原文件路径
        ...
        if retcode == 0:
            ...
            return retcode
        # 处理字幕
        return self.__transfer_subtitles(
            org_name=file_item, new_name=new_file, rmt_mode=rmt_mode
        )
    ...
    def transfer_media(
        ...
        def __finish_transfer(status, message):
            if status:
                self.progress.update(
                    ptype="filetransfer", value=100, text=f"{in_path} 转移成功！"
                )
            else:
                self.progress.update(
                    ptype="filetransfer",
            ...
            if os.path.isdir(in_path):
                ...
                if bluray_disk_dir:
                    ...
                    if str(min_filesize) == "0":
                        ...
                        now_filesize = (
                            self._min_filesize
                            if not str(min_filesize).isdigit()
        ...
        if in_from == SyncType.MON:
            ...
            if not file_list:
                log.info(
                    "【Rmt】所有文件均已成功转移过，没有需要处理的文件！如需重新处理，请清理缓存（服务->清理转移缓存）"
                )
        ...
        # 更新进度
        self.progress.update(
            ptype="filetransfer", text=f"共 {len(Medias)} 个文件需要处理..."
        )
        ...
        for file_item, media in Medias.items():
            try:
                ...
                if not media or not media.tmdb_info or not media.get_title_string():
                    ...
                    # 记录未识别
                    is_need_insert_unknown = (
                        self.dbhelper.is_need_insert_transfer_unknown(reg_path)
                    )
                    ...
                    if unknown_dir:
                        log.warn(
                            "【Rmt】%s 按原文件名转移到未识别目录：%s"
                            % (file_name, unknown_dir)
                        )
                        self.__transfer_origin_file(
                            file_item=file_item,
                ...
                if target_dir:
                    ...
                    dist_path = self.__get_best_target_path(
                ...
                dir_exist_flag, ret_dir_path, file_exist_flag, ret_file_path = (
                    self.__is_media_exists(dist_path, media)
                )
                # 新文件后缀
                file_ext = os.path.splitext(file_item)[-1]
                ...
                if dir_exist_flag:
                    ...
                    if file_exist_flag:
                        ...
                        if rmt_mode != RmtMode.SOFTLINK:
                            if (
                                media.size > os.path.getsize(ret_file_path)
                                and self._filesize_cover
                                or udf_flag
                            ):
                                ...
                                if ret != 0:
                                    ...
                                    self.progress.update(
                                        ptype="filetransfer", text=error_message
                                    )
                                    if udf_flag:
                                        return __finish_transfer(
                ...
                # 路径不存在
                    ...
                    if not ret_dir_path:
                        ...
                        failed_count += 1
                        if (
                            error_message not in alert_messages
                            and is_need_insert_unknown
                        ):
                            alert_messages.append(error_message)
                        continue
                    else:
        ...
        if download_subtitle_items:
            self.threadhelper.start_thread(
                Subtitle().download_subtitle, (download_subtitle_items,)
            )
        # 总结
        log.info(
            "【Rmt】%s 处理完成，总数：%s，失败：%s"
            % (in_path, total_count, failed_count)
        )
        if alert_count > 0:
            self.message.send_transfer_fail_message(
                in_path, alert_count, "、".join(alert_messages)
            )
        elif failed_count == 0:
            # 删除空目录
            if (
                ...
                log.info(
                    "【Rmt】目录下已无媒体文件及正在下载的文件，移动模式下删除目录：%s"
                    % in_path
                )
    ...
    def transfer_manually(self, s_path, t_path, mode):
        ...
        for path in PathUtils.get_dir_level1_medias(s_path, RMT_MEDIAEXT):
            ...
            ret, ret_msg = self.transfer_media(
                in_from=SyncType.MAN, in_path=path, target_dir=t_path, rmt_mode=rmt_mode
            )
            if not ret:
    ...
    def transfer_embyfav(self, item_path):
        ...
        if (
            movie_type == RMT_FAVTYPE
            or movie_type not in self.category.get_movie_categorys()
        ):
            return False
        movie_name = os.path.basename(movie_dir)
        movie_path = self.__get_best_target_path(
            mtype=MediaType.MOVIE, in_path=movie_dir
        )
        # 开始转移文件，转移到同目录下的精选目录
            ...
            for dest_path in dest_paths:
                if category_flag:
                    dest_path = os.path.join(
                        dest_path, meta_info.category, dir_name, season_name
                    )
                else:
                ...
                for file in files:
                    ...
                    if (
                        not file_meta_info.get_season_list()
    ...
    def link_sync_file(self, src_path, in_file, target_dir, sync_transfer_mode):
        ...
        return (
            self.__transfer_command(
                file_item=in_file, target_file=new_file, rmt_mode=sync_transfer_mode
            ),
            "",
        )
    ...
    def get_format_dict(self, media):
        ...
        return {
            ...
            "original_name": StringUtils.clear_file_name(
            ...
            "season_episode": "%s%s"
            % (media.get_season_item(), media.get_episode_items()),
            "part": media.part,
        }
    ...
    def get_moive_dest_path(self, media_info):
        ...
        dir_name = re.sub(
            r"[-_\s.]*None", "", self._movie_dir_rmt_format.format(**format_dict)
        )
        file_name = re.sub(
            r"[-_\s.]*None", "", self._movie_file_rmt_format.format(**format_dict)
        )
    ...
    def get_tv_dest_path(self, media_info):
        ...
        format_dict = self.get_format_dict(media_info)
        dir_name = re.sub(
            r"[-_\s.]*None", "", self._tv_dir_rmt_format.format(**format_dict)
        )
        season_name = re.sub(
            r"[-_\s.]*None", "", self._tv_season_rmt_format.format(**format_dict)
        )
        file_name = re.sub(
            r"[-_\s.]*None", "", self._tv_file_rmt_format.format(**format_dict)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--source", dest="s_path", required=True, help="硬链接源目录路径"
    )
    parser.add_argument(
        "-d", "--target", dest="t_path", required=False, help="硬链接目的目录路径"
    )
