...
from app.helper import DbHelper, SubmoduleHelper, ThreadHelper
...
from app.utils import ExceptionUtils, StringUtils, SystemUtils, Torrent
from app.utils.commons import singleton
from app.utils.types import DownloaderType, MediaType, RmtMode, SearchType
from config import PT_TAG, RMT_MEDIAEXT, Config
...
class Downloader:
    ...
    def __init__(self):
        self._downloader_schema = SubmoduleHelper.import_submodules(
            "app.downloader.client", filter_func=lambda _, obj: hasattr(obj, "schema")
        )
    ...
    def init_config(self):
        ...
        if pt:
            self._default_client_type = (
                ModuleConf.DOWNLOADER_DICT.get(pt.get("pt_client")) or DownloaderType.QB
            )
    ...
    def download(
        ...
        # 默认值
        _xpath, _hash, site_info, dl_files_folder, dl_files, retmsg = (
            None,
            False,
            {},
            "",
        ...
        if torrent_file:
            ...
            content, dl_files_folder, dl_files, retmsg = Torrent().read_torrent_content(
                torrent_file
            )
        # 没有种子文件解析链接
        else:
            ...
            if url.startswith("magnet:"):
                ...
                # 从详情页面XPATH解析下载链接
                if _xpath:
                    content = self.sites.parse_site_download_url(
                        page_url=url, xpath=_xpath
                    )
                    if not content:
                        return None, "无法从详情页面：%s 解析出下载链接" % url
                    # 解析出磁力链，补充Trackers
                    if content.startswith("magnet:"):
                        content = Torrent.add_trackers_to_magnet(
                            url=content, title=title
                        )
                    # 解析出来的是HASH值，转换为磁力链
                    elif _hash:
                        content = Torrent.convert_hash_to_magnet(
                            hash_text=content, title=title
                        )
                        if not content:
                ...
                # 从HTTP链接下载种子
                    ...
                    # 下载种子文件，并读取信息
                    _, content, dl_files_folder, dl_files, retmsg = (
        ...
        # 下载器类型
        dl_type = (
            self.__get_client_type(download_attr.get("downloader"))
        ...
        try:
            ...
            if is_paused:
                log.info(
                    "【Downloader】添加下载任务并暂停：%s，目录：%s，Url：%s"
                    % (title, download_dir, print_url)
                )
            else:
                log.info(
                    "【Downloader】添加下载任务：%s，目录：%s，Url：%s"
                    % (title, download_dir, print_url)
                )
            ...
            try:
                ...
                for task in trans_tasks:
                    ...
                    if not done_flag:
                        ...
                        self.default_client.set_torrents_status(
                            ids=task.get("id"), tags=task.get("tags")
                        )
                    else:
                        if self._pt_rmt_mode in [
                            RmtMode.MOVE,
                            RmtMode.RCLONE,
                            RmtMode.MINIO,
                        ]:
                            log.warn(
                                "【Downloader】移动模式下删除种子文件：%s"
                                % task.get("id")
                            )
    ...
    def get_downloading_torrents(self):
        ...
        try:
            return (
                self._default_client_type,
                self.default_client.get_downloading_torrents(tag=tag),
            )
                    ...
                    if need_tmdbid == item.tmdb_id:
                        if set(item_season).issubset(set(need_season)):
                            if len(item_season) == 1:
                                ...
                                torrent_episodes, torrent_path = (
                                    self.get_torrent_episodes(
                                        url=item.enclosure, page_url=item.page_url
                                    )
                                )
                                if not torrent_episodes or len(
                                    torrent_episodes
                                ) >= __get_season_episodes(need_tmdbid, item_season[0]):
                    ...
                    for item in download_list:
                        ...
                        if item.tmdb_id == need_tmdbid:
                            ...
                            if set(item_episodes).issubset(set(need_episodes)):
                                if __download(item):
                                    # 更新仍需集数
                                    need_episodes = __update_episodes(
                                        tmdbid=need_tmdbid,
                                        need=need_episodes,
                                        seq=index,
            ...
            for need_tmdbid in need_tv_list:
                ...
                for tv in need_tv:
                    ...
                    for item in download_list:
                        ...
                        if (
                            item.tmdb_id == need_tmdbid
                            and (
                                not item.get_episode_list()
                                or set(item.get_episode_list()).issuperset(
                                    set(need_episodes)
                                )
                            )
                            ...
                            selected_episodes = set(torrent_episodes).intersection(
                                set(need_episodes)
                            )
                            if not selected_episodes:
                                log.info(
                                    "【Downloader】%s 没有需要的集，跳过..."
                                    % item.org_string
                                )
                                continue
                            # 添加下载并暂停
                            ...
                            if downloader == DownloaderType.TR:
                                ...
                                torrent_id = _client.get_torrent_id_by_tag(
                                    tag=torrent_tag, status=["paused"]
                                )
                            else:
                                continue
                            if not torrent_id:
                                log.error(
                                    "【Downloader】获取下载器添加的任务信息出错：%s，tag=%s"
                                    % (item.org_string, torrent_tag)
                                )
                                continue
                            # 设置任务只下载想要的文件
                            log.info(
                                "【Downloader】从 %s 中选取集：%s"
                                % (item.org_string, selected_episodes)
                            )
                            self.set_files_status(
    ...
    def check_exists_medias(self, meta_info, no_exists=None, total_ep=None):
        ...
        if meta_info.type != MediaType.MOVIE:
            ...
            if tv_info:
                ...
                if not total_seasons:
                    ...
                    for season in total_seasons:
                        ...
                        if no_exists_episodes:
                            ...
                            if search_episode:
                                # 有集数，肯定只有一季
                                if not set(search_episode).intersection(
                                    set(no_exists_episodes)
                                ):
                                    # 搜索的跟不存在的没有交集，说明都存在了
                                    msg = (
                                        f"媒体库中已存在剧集：\n"
                ...
                log.info(
                    "【Downloader】%s 无法查询到媒体详细信息"
                    % meta_info.get_title_string()
                )
                message_list.append(
                    "%s 无法查询到媒体详细信息" % meta_info.get_title_string()
                )
                return_flag = None
            # 全部存在
        ...
        if downloader == DownloaderType.TR:
            ...
            for file_id, torrent_file in enumerate(torrent_files):
                ...
                if not meta_info.get_episode_list():
                    ...
                    selected = set(meta_info.get_episode_list()).issubset(
                        set(need_episodes)
                    )
                    if selected:
                        sucess_epidised = list(
                            set(sucess_epidised).union(
            ...
            for torrent_file in torrent_files:
                ...
                if not meta_info.get_episode_list() or not set(
                    meta_info.get_episode_list()
                ).issubset(set(need_episodes)):
                    file_ids.append(torrent_file.get("index"))
                else:
                    sucess_epidised = list(
                        set(sucess_epidised).union(set(meta_info.get_episode_list()))
                    )
    ...
    def get_download_dirs(self, setting=None):
        ...
        # 查询目录
        save_path_list = [
            attr.get("save_path") for attr in self._downloaddir if attr.get("save_path")
        ]
        save_path_list.sort()
        ...
        if media and media.tmdb_info:
            for attr in self._downloaddir or []:
                ...
                if (
                    ...
                    and media.size
                    and float(
                        SystemUtils.get_free_space_gb(
                            attr.get("container_path") or attr.get("save_path")
                        )
                    )
                    < float(
                        int(StringUtils.num_filesize(media.size)) / 1024 / 1024 / 1024
                    )
                ):
                    continue
    ...
    def get_default_download_setting(self):
        ...
        default_download_setting = (
            SystemConfig().get_system_config("DefaultDownloadSetting") or "-1"
        )
