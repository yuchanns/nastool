...
class NexusPhpSiteUserInfo(_ISiteUserInfo):
    ...
    def _parse_user_base_info(self, html_text):
        ...
        ret = html.xpath(
            f'//a[contains(@href, "userdetails") and contains(@href, "{self.userid}")]//b//text()'
        )
        if ret:
    ...
    def __parse_user_traffic_info(self, html_text):
        ...
        leeching_match = re.search(
            r"(Torrents leeching|下载中)[\u4E00-\u9FA5\D\s]+(\d+)[\s\S]+<", html_text
        )
        self.leeching = (
            StringUtils.str_int(leeching_match.group(2))
    ...
    def _parse_user_detail_info(self, html_text):
        ...
        seeding_sizes = html.xpath(
            '//tr/td[text()="当前上传"]/following-sibling::td[1]//'
        ...
        if seeding_sizes:
            ...
            tmp_seeding = (
                StringUtils.str_int(seeding_match.group(1))
                if (seeding_match and seeding_match.group(1))
                else 0
            )
            tmp_seeding_size = (
                StringUtils.num_filesize(seeding_size_match.group(1).strip())
    ...
    def __fixup_torrent_seeding_page(self, html):
        ...
        # 从JS调用种获取用户ID
        seeding_url_text = html.xpath(
            '//a[contains(@href, "javascript: getusertorrentlistajax") '
            'and contains(@href,"seeding")]/@href'
        )
        csrf_text = html.xpath('//meta[@name="x-csrf"]/@content')
        if not self._torrent_seeding_page and seeding_url_text:
            user_js = re.search(
                r"javascript: getusertorrentlistajax\(\s*'(\d+)", seeding_url_text[0]
            )
            ...
            if csrf_text[0].strip():
                self._torrent_seeding_page = "ajax_getusertorrentlist.php"
                self._torrent_seeding_params = {
                    "userid": self.userid,
                    "type": "seeding",
                    "csrf": csrf_text[0].strip(),
                }
    ...
    def __get_user_level(self, html):
        ...
        user_levels_text = html.xpath(
            '//tr/td[text()="等級" or text()="等级" or *[text()="等级"]]/'
    ...
    def _parse_message_unread_links(self, html_text, msg_links):
        ...
        message_links = html.xpath(
            '//tr[not(./td/img[@alt="Read"])]/td/a[contains(@href, "viewmessage")]/@href'
        )
    ...
    def _parse_message_content(self, html_text):
        ...
        message_date = html.xpath(
            '//h1/following-sibling::table[.//tr/td[@class="colhead"]]//tr[2]/td[2]'
        ...
        message_content = html.xpath(
            '//h1/following-sibling::table[.//tr/td[@class="colhead"]]//tr[3]/td'
