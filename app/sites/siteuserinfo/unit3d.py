...
class Unit3dSiteUserInfo(_ISiteUserInfo):
        ...
        if tmps:
            ...
            if user_name_match and user_name_match.group().strip():
                ...
                self._torrent_seeding_page = (
                    f"/users/{self.username}/active?perPage=100&client=&seeding=include"
                )
    ...
    def _parse_user_detail_info(self, html_text):
        ...
        user_levels_text = html.xpath(
            '//div[contains(@class, "content")]//span[contains(@class, "badge-user")]/text()'
        )
        if user_levels_text:
    ...
    def _parse_user_torrent_seeding_info(self, html_text, multi_page=False):
        ...
        if html.xpath('//tr[position()=1]/th[contains(@class,"size")]'):
            size_col = (
                len(
                    html.xpath(
                        '//tr[position()=1]/th[contains(@class,"size")]/preceding-sibling::th'
                    )
                )
                + 1
            )
        ...
        if html.xpath('//tr[position()=1]/th[contains(@class,"seeders")]'):
            seeders_col = (
                len(
                    html.xpath(
                        '//tr[position()=1]/th[contains(@class,"seeders")]/preceding-sibling::th'
                    )
                )
                + 1
            )
        ...
        # 是否存在下页数据
        next_page = None
        next_pages = html.xpath(
            '//ul[@class="pagination"]/li[contains(@class,"active")]/following-sibling::li'
        )
