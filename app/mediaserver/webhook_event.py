...
class WebhookEvent:
    ...
    def __parse_emby_msg(message):
        ...
        if message.get("Item"):
            if message.get("Item", {}).get("Type") == "Episode":
                ...
                eventItem["percentage"] = message.get("TranscodingInfo", {}).get(
    ...
    def send_webhook_message(self, event_info, channel):
        ...
        if self.is_ignore_webhook_message(
            event_info.get("user_name"), event_info.get("device_name")
        ):
            return
        ...
        if event_info.get("item_id"):
            ...
            if not image_url:
                image_url = self.mediaserver.get_image_by_id(
                    event_info.get("item_id"), "Backdrop"
                ) or _webhook_images.get(channel)
        else:
    ...
    def is_ignore_webhook_message(self, user_name, device_name):
        ...
        if not webhook_ignore:
            return False
        if (
            user_name in webhook_ignore
            or device_name in webhook_ignore
            or (user_name + ":" + device_name) in webhook_ignore
        ):
            return True
        return False
