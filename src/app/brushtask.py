...
from app.utils import ExceptionUtils, StringUtils, Torrent
...
class BrushTask(object):
    ...
    def init_config(self):
        ...
        if task_flag:
            self._scheduler.add_job(
                func=self.remove_tasks_torrents,
                trigger="interval",
    ...
    def remove_tasks_torrents(self):
        ...
        def __send_message(_task_name, _delete_type, _torrent_name):
            ...
            _msg_text = "删除原因：{}\n种子名称：{}".format(
                _delete_type.value, _torrent_name
            )
        ...
        for taskinfo in self._brush_tasks:
            ...
            try:
                ...
                if client_type == self._qb_client:
                    ...
                    # 检查完成状态的
                    torrents, has_err = downloader.get_torrents(
                        ids=torrent_ids, status=["completed"]
                    )
                    # 看看是否有错误, 有错误的话就不处理了
                    if has_err:
                    ...
                    remove_torrent_ids = list(
                        set(torrent_ids).difference(
                            set([torrent.get("hash") for torrent in torrents])
                        )
                    )
                    for torrent in torrents:
                        ...
                        need_delete, delete_type = self.__check_remove_rule(
                        ...
                        if need_delete:
                            log.info(
                                "【Brush】%s 做种达到删种条件：%s，删除任务..."
                                % (torrent.get("name"), delete_type.value)
                            )
                            if sendmessage:
                                __send_message(
                                    task_name, delete_type, torrent.get("name")
                                )

                            if torrent_id not in delete_ids:
                                delete_ids.append(torrent_id)
                                update_torrents.append(
                                    (
                                        "%s,%s" % (uploaded, downloaded),
                                        taskid,
                                        torrent_id,
                                    )
                                )
                    # 检查下载中状态的
                    torrents, has_err = downloader.get_torrents(
                        ids=torrent_ids, status=["downloading"]
                    )
                    # 看看是否有错误, 有错误的话就不处理了
                    if has_err:
                        log.warn("【BRUSH】任务 %s 获取种子状态失败" % task_name)
                ...
                # transmission
                    ...
                    torrents, has_err = downloader.get_torrents(
                        ids=torrent_ids, status=["seeding", "seed_pending"]
                    )
                    # 看看是否有错误, 有错误的话就不处理了
                    if has_err:
                        log.warn("【BRUSH】任务 %s 获取种子状态失败" % task_name)
                    ...
                    for torrent in torrents:
                        ...
                        date_now = int(time.mktime(datetime.now().timetuple()))
                        dltime = date_now - int(
                            time.mktime(torrent.date_added.timetuple())
                        )
                        seeding_time = date_now - int(
                            time.mktime(date_done.timetuple())
                        )
                        # 下载量
                        ...
                        if need_delete:
                            log.info(
                                "【Brush】%s 做种达到删种条件：%s，删除任务..."
                                % (torrent.name, delete_type.value)
                            )
                            if sendmessage:
                    ...
                    for torrent in torrents:
                        ...
                        # 下载耗时
                        dltime = (
                            datetime.now().astimezone() - torrent.date_added
                        ).seconds
                        # 下载量
                        downloaded = int(torrent.total_size * torrent.progress / 100)
                        total_downloaded += downloaded
                ...
                if remove_torrent_ids:
                    log.info(
                        "【Brush】任务 %s 的这些下载任务在下载器中不存在，将删除任务记录：%s"
    ...
    def __is_allow_new_torrent(
        self, taskid, taskname, downloadercfg, seedsize, dlcount
    ):
        """
        检查是否还能添加新的下载
        """
        ...
        if dlcount:
            ...
            if downloading_count is None:
                log.error(
                    "【Brush】任务 %s 下载器 %s 无法连接"
                    % (taskname, downloadercfg.get("name"))
                )
                return False
    ...
    def __download_torrent(
        ...
        if not download_id:
            ...
            if sendmessage:
                ...
                msg_text = "种子名称：{}\n种子大小：{}".format(
                    title, StringUtils.str_filesize(size)
                )
                self.message.send_brushtask_added_message(
                    title=msg_title, text=msg_text
                )
        # 插入种子数据
        if self.dbhelper.insert_brushtask_torrent(
            brush_id=taskid,
            title=title,
            enclosure=enclosure,
    ...
    def __check_rss_rule(
        self, rss_rule, title, torrent_url, torrent_size, pubdate, cookie, ua, proxy
    ):
        """
        检查种子是否符合刷流过滤条件
        ...
        try:
            if rss_rule.get("size"):
                ...
                if rule_sizes[0]:
                    if len(rule_sizes) > 1 and rule_sizes[1]:
                        ...
                        if (
                            rule_sizes[0] == "gt"
                            and float(torrent_size) < float(min_size) * 1024**3
                        ):
                            return False
                        if (
                            rule_sizes[0] == "lt"
                        ...
                        if (
                            rule_sizes[0] == "bw"
                            and not float(min_size) * 1024**3
                            < float(torrent_size)
                            < float(max_size) * 1024**3
                        ):
                            return False
            ...
            torrent_attr = self.sites.check_torrent_attr(
                torrent_url=torrent_url, cookie=cookie, ua=ua, proxy=proxy
            )
            ...
            if rss_rule.get("peercount"):
                ...
                if len(peer_counts) >= 2 and peer_counts[1]:
                    ...
                    if peer_counts[0] == "gt" and torrent_peer_count <= min_count:
                        log.debug(
                            "【Brush】%s `判断做种数, 判断条件: peer_count:%d %s threshold:%d"
                    ...
                    if peer_counts[0] == "lt" and torrent_peer_count >= min_count:
                        ...
                        return False
                    if peer_counts[0] == "bw" and not (
                        min_count <= torrent_peer_count <= max_count
                    ):
                        log.debug(
                            "【Brush】%s `判断做种数, 判断条件: left:%d %s peer_count:%d %s right:%d"
                            % (
                                title,
                                min_count,
                                peer_counts[0],
            ...
            if rss_rule.get("pubdate") and pubdate:
                ...
                if len(rule_pubdates) >= 2 and rule_pubdates[1]:
                    ...
                    log.debug(
                        "【Brush】发布时间：%s，当前时间：%s"
                        % (localpubdate.isoformat(), localnowtime.isoformat())
                    )
                    if (localnowtime - localpubdate).seconds / 3600 > float(
                        rule_pubdates[1]
                    ):
