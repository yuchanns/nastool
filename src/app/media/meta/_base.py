...
class MetaBase(object):
    ...
    _subtitle_season_all_re = (
        r"全\s*([0-9一二三四五六七八九十]+)\s*季|([0-9一二三四五六七八九十]+)\s*季全"
    )
    ...
    def get_overview_string(self, max_len=140):
        ...
        overview = (
            (overview[:max_len] + placeholder) if len(overview) > max_len else overview
        )
        return overview
    ...
    def get_episode_list(self):
        if self.begin_episode is None:
            ...
            return [
                episode for episode in range(self.begin_episode, self.end_episode + 1)
            ]
        else:
    ...
    def get_backdrop_image(self, default=True, original=False):
        if self.fanart_backdrop:
            ...
            self.fanart_backdrop = self.fanart.get_backdrop(
    ...
    def get_poster_image(self, original=False):
        ...
        if not self.fanart_poster:
            self.fanart_poster = self.fanart.get_poster(
                media_type=self.type,
                queryid=self.tmdb_id if self.type == MediaType.MOVIE else self.tvdb_id,
            )
    ...
    def get_volume_factor_string(self):
        return self.get_free_string(
            self.upload_volume_factor, self.download_volume_factor
        )
    ...
    def set_tmdb_info(self, info):
        ...
        self.tmdb_info = info
        self.vote_average = (
            round(float(info.get("vote_average")), 1) if info.get("vote_average") else 0
        )
        ...
        self.poster_path = (
            TMDB_IMAGE_W500_URL % info.get("poster_path")
            if info.get("poster_path")
            else ""
        )
        self.backdrop_path = (
            TMDB_IMAGE_W500_URL % info.get("backdrop_path")
    ...
    def init_subtitle(self, title_text):
        ...
        if re.search(r"[全第季集话話期]", title_text, re.IGNORECASE):
            # 第x季
            season_str = re.search(
                r"%s" % self._subtitle_season_re, title_text, re.IGNORECASE
            )
            if season_str:
                ...
                try:
                    ...
                    if seasons.find("-") != -1:
                        ...
                        begin_season = int(
                            cn2an.cn2an(seasons[0].strip(), mode="smart")
                        )
                        if len(seasons) > 1:
                            end_season = int(
                                cn2an.cn2an(seasons[1].strip(), mode="smart")
                            )
                    else:
            ...
            # 第x集
            episode_str = re.search(
                r"%s" % self._subtitle_episode_re, title_text, re.IGNORECASE
            )
            if episode_str:
                episodes = episode_str.group(1)
                if episodes:
                    episodes = (
                        episodes.upper().replace("E", "").replace("P", "").strip()
                    )
                else:
                    return
                try:
                    ...
                    if episodes.find("-") != -1:
                        ...
                        begin_episode = int(
                            cn2an.cn2an(episodes[0].strip(), mode="smart")
                        )
                        if len(episodes) > 1:
                            end_episode = int(
                                cn2an.cn2an(episodes[1].strip(), mode="smart")
                            )
                    else:
                ...
                self._subtitle_flag = True
            # x集全
            episode_all_str = re.search(
                r"%s" % self._subtitle_episode_all_re, title_text, re.IGNORECASE
            )
            if episode_all_str:
                ...
                self.type = MediaType.TV
            # 全x季 x季全
            season_all_str = re.search(
                r"%s" % self._subtitle_season_all_re, title_text, re.IGNORECASE
            )
            if season_all_str:
