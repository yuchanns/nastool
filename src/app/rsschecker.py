...
class RssChecker(object):
            ...
            self._rss_tasks.append(
                {
                    ...
                    "sites": (
                        json.loads(task.SITES)
                        if task.SITES
                        else {"rss_sites": [], "search_sites": []}
                    ),
                    "filter_args": (
                        json.loads(task.FILTER_ARGS)
                        if task.FILTER_ARGS
    ...
    def check_task_rss(self, taskid):
        ...
        if len(rss_result) == 0:
            ...
            log.info(
                "【RssChecker】%s 获取数据：%s"
                % (taskinfo.get("name"), len(rss_result))
            )
        # 处理RSS结果
        res_num = 0
        no_exists = {}
        for res in rss_result:
            try:
                ...
                if mediatype:
                    mediatype = (
                        MediaType.MOVIE if mediatype == "movie" else MediaType.TV
                    )
                ...
                if taskinfo.get("uses") == "D":
                    ...
                    if taskinfo.get("recognization") == "Y":
                        if cache_info.get("id"):
                            ...
                            media_info = self.media.get_media_info(
                                title=meta_name, mtype=mediatype
                            )
                            if not media_info:
                            ...
                            if not media_info.tmdb_info:
                                media_info.set_tmdb_info(
                                    self.media.get_tmdb_info(
                                        mtype=media_info.type, tmdbid=media_info.tmdb_id
                                    )
                                )
                            # TMDB信息插入订阅任务
                            if media_info.type != MediaType.MOVIE:
                                self.dbhelper.insert_userrss_mediainfos(
                                    taskid, media_info
                                )
                        else:
                    ...
                    if not enclosure:
                        log.warn(
                            "【RssChecker】%s RSS报文中没有enclosure种子链接"
                            % taskinfo.get("name")
                        )
                        continue
                ...
                ExceptionUtils.exception_traceback(e)
                log.error(
                    "【RssChecker】处理RSS发生错误：%s - %s"
                    % (str(e), traceback.format_exc())
                )
                continue
        log.info(
            "【RssChecker】%s 处理结束，匹配到 %s 个有效资源"
            % (taskinfo.get("name"), res_num)
        )
        # 添加下载
        if rss_download_torrents:
            for media in rss_download_torrents:
                ret, ret_msg = self.downloader.download(
                    media_info=media,
                ...
                if ret:
                    self.message.send_download_message(
                        in_from=SearchType.USERRSS, can_item=media
                    )
                    # 下载类型的 这里下载成功了 插入数据库
                    ...
                    if media.download_setting:
                        download_attr = self.downloader.get_download_setting(
                            media.download_setting
                        )
                        if download_attr.get("downloader"):
                            downloader = download_attr.get("downloader")
                    self.dbhelper.insert_userrss_task_history(
                        taskid, media.org_string, downloader
                    )
                else:
                    log.error(
                        "【RssChecker】添加下载任务 %s 失败：%s"
                        % (
                            media.get_title_string(),
                            ret_msg or "请检查下载任务是否已存在",
                        )
                    )
                    if ret_msg:
            ...
            for media in rss_subscribe_torrents:
                ...
                if rss_media and code == 0:
                    self.message.send_rss_success_message(
                        in_from=SearchType.USERRSS, media_info=rss_media
                    )
                else:
                    log.warn(
                        "【RssChecker】%s 添加订阅失败：%s" % (media.get_name(), msg)
                    )

        # 更新状态
        counter = (
            len(rss_download_torrents)
            + len(rss_subscribe_torrents)
            + len(rss_search_torrents)
        )
        if counter:
    ...
    def __parse_userrss_result(self, taskinfo):
        ...
        try:
            ...
            ExceptionUtils.exception_traceback(e)
            log.error(
                "【RssChecker】任务 %s 的解析配置不是合法的Json格式"
                % taskinfo.get("name")
            )
            return []
        # 拼装链接
        ...
        if rss_parser.get("params"):
            ...
            try:
                ...
                ExceptionUtils.exception_traceback(e)
                log.error(
                    "【RssChecker】任务 %s 的解析配置附加参数不合法"
                    % taskinfo.get("name")
                )
                return []
            rss_url = (
                "%s?%s" % (rss_url, param_url)
                if rss_url.find("?") == -1
                else "%s&%s" % (rss_url, param_url)
            )
        # 请求数据
        try:
        ...
        if rss_parser.get("type") == "XML":
            try:
                ...
                log.error(
                    "【RssChecker】任务 %s 获取的订阅报文无法解析：%s"
                    % (taskinfo.get("name"), str(err))
                )
                return []
            ...
            try:
                ...
                ExceptionUtils.exception_traceback(err)
                log.error(
                    "【RssChecker】任务 %s 获取的订阅报文不是合法的Json格式：%s"
                    % (taskinfo.get("name"), str(err))
                )
                return []
            ...
            if not isinstance(item_list, list):
                log.error(
                    "【RssChecker】任务 %s 获取的订阅报文list后不是列表"
                    % taskinfo.get("name")
                )
                return []
            for item in item_list:
    ...
    def test_rss_articles(self, taskid, title):
        ...
        if not media_info.tmdb_id:
            ...
            if media_info.type == MediaType.MOVIE:
                ...
                if exist_flag:
                    log.info(
                        "【RssChecker】电影 %s 已存在" % media_info.get_title_string()
                    )
            else:
                exist_flag, no_exists, _ = self.downloader.check_exists_medias(
                    meta_info=media_info, no_exists=no_exists
    ...
    def check_rss_articles(self, flag, articles):
        ...
        try:
            if flag == "set_finished":
                ...
                for article in articles:
                    self.dbhelper.simple_delete_rss_torrents(
                        article.get("title"), article.get("enclosure")
                    )
            else:
                return False
            return True
