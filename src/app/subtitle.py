...
from app.utils import ExceptionUtils, PathUtils, RequestUtils, StringUtils, SystemUtils
    ...
    def init_config(self):
        ...
        if subtitle:
            ...
            if self._server == "chinesesubfinder":
                ...
                self._local_path = subtitle.get("chinesesubfinder", {}).get(
                    "local_path"
                )
                self._remote_path = subtitle.get("chinesesubfinder", {}).get(
                    "remote_path"
                )
            else:
                self._opensubtitles_enable = subtitle.get("opensubtitles", {}).get(
                    "enable"
                )
    ...
    def __download_opensubtitles(self, items):
        ...
        for item in items:
            ...
            if subtitles is None:
                log.info(
                    "【Subtitle】开始从Opensubtitle.org检索字幕: %s，imdbid=%s"
                    % (item.get("name"), item.get("imdbid"))
                )
                ...
                if not subtitles:
                    ...
                    log.info(
                        "【Subtitle】opensubtitles.org返回数据：%s" % len(subtitles)
                    )
            if not subtitles:
                continue
            # 成功数
            ...
            for subtitle in subtitles:
                ...
                if not item.get("imdbid"):
                    if str(subtitle.get("title")) != "%s (%s)" % (
                        item.get("name"),
                        item.get("year"),
                    ):
                        continue
                # 季
                if item.get("season") and str(
                    subtitle.get("season").replace("Season", "").strip()
                ) != str(item.get("season")):
                    continue
                # 集
                if item.get("episode") and str(subtitle.get("episode")) != str(
                    item.get("episode")
                ):
                    continue
                # 字幕文件名
                ...
                log.info(
                    "【Subtitle】正在从opensubtitles.org下载字幕 %s 到 %s "
                    % (SubFileName, Media_File)
                )
                # 下载
                ret = RequestUtils(
                    cookies=self.opensubtitles.get_cookie(),
                    headers=self.opensubtitles.get_ua(),
                ).get_res(Download_Link)
                if ret and ret.status_code == 200:
                    # 保存ZIP
                    file_name = self.__get_url_subtitle_name(
                        ret.headers.get("content-disposition"), Download_Link
                    )
                    if not file_name:
                        continue
                    ...
                    # 遍历转移文件
                    for sub_file in PathUtils.get_dir_files(
                        in_path=zip_path, exts=RMT_SUBEXT
                    ):
            ...
            if not subtitle_count:
                if item.get("episode"):
                    log.info(
                        "【Subtitle】%s 第%s季 第%s集 未找到符合条件的字幕"
                ...
                log.info(
                    "【Subtitle】%s 共下载了 %s 个字幕"
                    % (item.get("name"), subtitle_count)
                )
    ...
    def __download_chinesesubfinder(self, items):
        ...
        for item in items:
            ...
            if (
                ...
                and file_path.startswith(self._local_path)
            ):
                ...
                try:
                    ...
                    if not res or res.status_code != 200:
                        ...
                        if res.text:
                            ...
                            if not job_id:
                                log.warn(
                                    "【Subtitle】ChineseSubFinder下载字幕出错：%s"
                                    % message
                                )
                                ret_msg = "ChineseSubFinder下载字幕出错：%s" % message
                            else:
                                log.info(
                                    "【Subtitle】ChineseSubFinder任务添加成功：%s"
                                    % job_id
                                )
        ...
        if res and res.status_code == 200:
            ...
            for sublink in sublink_list:
                ...
                if ret and ret.status_code == 200:
                    ...
                    # 保存ZIP
                    file_name = self.__get_url_subtitle_name(
                        ret.headers.get("content-disposition"), sublink
                    )
                    if not file_name:
                    ...
                    if file_name.lower().endswith(".zip"):
                        ...
                        for sub_file in PathUtils.get_dir_files(
                            ...
                            log.info(
                                f"【Subtitle】转移字幕 {sub_file} 到 {target_sub_file}"
                            )
            ...
            log.warn(
                f"【Subtitle】连接 {media_info.page_url} 失败，状态码：{res.status_code}"
            )
        else:
