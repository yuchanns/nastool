...
class TorrentSpider(feapder.AirSpider):
    ...
    def start_requests(self):
        ...
        if self.keyword:
            ...
            if self.search.get("params"):
                ...
                # 查询参数
                params = {"search_mode": search_mode, "page": self.page or 0}
                # 额外参数
                ...
                if self.category:
                    if self.mtype == MediaType.MOVIE:
                        ...
                        cats = (
                            self.category.get("movie")
                            or [] + self.category.get("tv")
                            or []
                        )
                    for cat in cats:
        ...
        # 列表浏览
            ...
            if self.browse:
                ...
                if self.browse.get("start"):
                    inputs_dict.update({"page": self.browse.get("start")})
            elif self.page:
    ...
    def download_midware(self, request):
        request.headers = {"User-Agent": self.ua, "Cookie": self.cookie}
        if self.proxies:
    ...
    def Gettitle_default(self, torrent):
        ...
        if "selector" in selector:
            ...
            if "attribute" in selector:
                items = [
                    item.attr(selector.get("attribute"))
            ...
            if items:
                if "contents" in selector and len(items) > int(
            ...
            if "title_default" in self.fields:
                ...
                if "attribute" in title_default_selector:
                    render_dict.update(
                        {
                            "title_default": title_default_item.attr(
                                title_default_selector.get("attribute")
                            )
                        }
                    )
                else:
            ...
            if "title_optional" in self.fields:
                ...
                if "attribute" in title_optional_selector:
                    render_dict.update(
                        {
                            "title_optional": title_optional_item.attr(
                                title_optional_selector.get("attribute")
                            )
                        }
                    )
                else:
    ...
    def Gettitle_optional(self, torrent):
        ...
        if "selector" in selector or "selectors" in selector:
            ...
            if description:
                ...
                if "attribute" in selector:
                    items = [
                        x.attr(selector.get("attribute")) for x in description.items()
                    ]
                else:
                ...
                if items:
                    if "contents" in selector and len(items) > int(
                        ...
                        items = items[0].split("\n")[selector.get("contents")]
                    elif "index" in selector and len(items) > int(
                        selector.get("index")
                    ):
                        items = items[int(selector.get("index"))]
                    else:
                        items = items[0]
                    self.torrents_info["description"] = (
                        items if not isinstance(items, list) else items[0]
                    )
        elif "text" in selector:
            render_dict = {}
            ...
            if "description_free_forever" in self.fields:
                render_dict.update(
                    {
                        "description_free_forever": torrent(
                            self.fields.get("description_free_forever", {}).get(
                                "selector", ""
                            )
                        ).text()
                    }
            ...
            if "description_normal" in self.fields:
                render_dict.update(
                    {
                        "description_normal": torrent(
                            self.fields.get("description_normal", {}).get(
                                "selector", ""
                            )
                        ).text()
                    }
    ...
    def Getdetails(self, torrent):
        ...
        items = [
            item.attr(self.fields.get("details", {}).get("attribute"))
        ...
        if items:
            ...
            if "filters" in self.fields.get("details", {}):
                self.torrents_info["page_url"] = self.__filter_text(
                    self.torrents_info.get("page_url"),
    ...
    def Getdownload(self, torrent):
        ...
        if "detail" in self.fields.get("download", {}):
            ...
            items = [
                item.attr(self.fields.get("download", {}).get("attribute"))
                for item in download.items()
            ]
            if items:
                if not items[0].startswith("http") and not items[0].startswith(
                    "magnet"
                ):
                    self.torrents_info["enclosure"] = (
                        self.domain + items[0][1:]
                        if items[0].startswith("/")
    ...
    def Getimdbid(self, torrent):
        ...
        if "attribute" in selector:
            items = [
                item.attr(selector.get("attribute")) for item in imdbid.items() if item
            ]
        else:
    ...
    def Getsize(self, torrent):
        ...
        if "index" in selector and len(items) > selector.get("index"):
    ...
    def Getpubdate(self, torrent):
        ...
        if "attribute" in selector:
            items = [
                item.attr(selector.get("attribute")) for item in pubdate.items() if item
            ]
        else:
    ...
    def Getelapsed_date(self, torrent):
        ...
        if "attribute" in selector:
            items = [
                item.attr(selector.get("attribute"))
                for item in date_elapsed.items()
                if item
            ]
        else:
