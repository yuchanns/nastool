...
class UserManage(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "oper", type=str, help="操作类型（add 新增/del删除）", location="form", required=True
    )
...
class SystemProgress(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "type", type=str, help="类型（search/mediasync）", location="form", required=True
    )
...
class RssParserList(ClientResource):
    ...
    def post():
        ...
        查询所有解析器
        """
        return {"code": 0, "success": True, "data": {"parsers": RssChecker().get_userrss_parser()}}
...
class RssItemSet(ClientResource):
    ...
    parser.add_argument(
        "articles", type=list, help="条目（{title/enclosure}）", location="form", required=True
    )
...
class MediaCategoryList(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "type", type=str, help="类型（电影/电视剧/动漫）", location="form", required=True
    )
...
class BrushTaskUpdate(ClientResource):
    ...
    parser.add_argument(
        "brushtask_interval", type=int, help="刷新间隔(分钟)", location="form", required=True
    )
    parser.add_argument(
        "brushtask_downloader", type=int, help="下载器", location="form", required=True
    )
    parser.add_argument(
        "brushtask_totalsize", type=int, help="保种体积(GB)", location="form", required=True
    )
    parser.add_argument(
...
class BrushTaskList(ClientResource):
    ...
    def post():
        ...
        查询所有刷流任务
        """
        return {"code": 0, "success": True, "data": {"tasks": BrushTask().get_brushtask_info()}}
...
class BrushTaskDownloaderUpdate(ClientResource):
    ...
    parser.add_argument("name", type=str, help="名称", location="form", required=True)
    parser.add_argument(
        "type", type=str, help="类型（qbittorrent/transmission）", location="form", required=True
    )
...
class WordItemUpdate(ClientResource):
    ...
    parser.add_argument("gid", type=int, help="识别词组ID", location="form", required=True)
    parser.add_argument(
        "group_type", type=str, help="媒体类型（1-电影/2-电视剧）", location="form", required=True
    )
    ...
    parser.add_argument(
        "enabled", type=str, help="状态（1-启用/0-停用）", location="form", required=True
    )
...
class MessageClientUpdate(ClientResource):
    ...
    parser.add_argument(
        "interactive", type=int, help="是否开启交互（0/1）", location="form", required=True
    )
...
class MessageClientStatus(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "flag", type=str, help="操作类型（interactive/enable）", location="form", required=True
    )
    parser.add_argument("cid", type=int, help="ID", location="form", required=True)
...
class TorrentRemoverTaskUpdate(ClientResource):
    ...
    parser.add_argument(
        "enabled", type=int, help="状态（0-停用/1-启用）", location="form", required=True
    )
    parser.add_argument(
        "samedata", type=int, help="处理辅种（0-否/1-是）", location="form", required=True
    )
    parser.add_argument(
        "onlynastool",
        type=int,
