...
from web.security import generate_access_token, login_required, require_auth
...
class UserLogin(Resource):
    ...
    parser.add_argument(
        "username", type=str, help="用户名", location="form", required=True
    )
    parser.add_argument(
        "password", type=str, help="密码", location="form", required=True
    )
...
class ServiceNetworkTest(ClientResource):
    ...
    def post(self):
        ...
        网络连接性测试
        """
        return WebAction().api_action(
            cmd="net_test", data=self.parser.parse_args().get("url")
        )
...
class SiteStatistic(ApiResource):
    ...
    def get():
        ...
        return {
            ...
            "success": True,
            "data": {
                "user_statistics": WebAction()
                .get_site_user_statistics({"encoding": "DICT"})
                .get("data")
            },
        }
...
class SiteSites(ApiResource):
    ...
    def get():
        ...
        获取所有站点配置（密钥认证）
        """
        return {"code": 0, "success": True, "data": {"user_sites": Sites().get_sites()}}
...
class SiteUpdate(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "site_name", type=str, help="站点名称", location="form", required=True
    )
...
class SiteFavicon(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "name", type=str, help="站点名称", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_site_favicon", data=self.parser.parse_args()
        )
...
class SiteStatisticsActivity(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_site_activity", data=self.parser.parse_args()
        )
...
class SiteCheck(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "url", type=str, help="站点地址", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="check_site_attr", data=self.parser.parse_args()
        )
...
class SiteStatisticsHistory(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "days", type=int, help="时间范围（天）", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_site_history", data=self.parser.parse_args()
        )
...
class SiteStatisticsSeedinfo(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_site_seeding_info", data=self.parser.parse_args()
        )
...
class SiteResources(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "id", type=str, help="站点索引ID", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="list_site_resources", data=self.parser.parse_args()
        )
...
class SiteList(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "basic", type=int, help="只查询基本信息（0-否/1-是）", location="form"
    )
    ...
    parser.add_argument(
        "statistic", type=int, help="数据统计（0-否/1-是）", location="form"
    )
...
class SearchKeyword(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "search_word", type=str, help="搜索关键字", location="form", required=True
    )
    parser.add_argument(
        "unident", type=int, help="快速模式（0-否/1-是）", location="form"
    )
    ...
    parser.add_argument(
        "media_type", type=str, help="类型（电影/电视剧）", location="form"
    )
...
class DownloadSearch(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "id", type=str, help="搜索结果ID", location="form", required=True
    )
...
class DownloadItem(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "enclosure", type=str, help="链接URL", location="form", required=True
    )
    ...
    parser.add_argument(
        "uploadvolumefactor", type=float, help="上传因子", location="form"
    )
    parser.add_argument(
        "downloadvolumefactor", type=float, help="下载因子", location="form"
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="download_link", data=self.parser.parse_args()
        )
...
class DownloadHistory(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_downloaded", data=self.parser.parse_args()
        )
...
class DownloadConfigInfo(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "sid", type=str, help="下载设置ID", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_download_setting", data=self.parser.parse_args()
        )
...
class DownloadConfigUpdate(ClientResource):
    ...
    parser.add_argument(
        "content_layout",
        type=int,
        help="布局（0-全局/1-原始/2-创建子文件夹/3-不建子文件夹）",
        location="form",
    )
    parser.add_argument(
        "is_paused", type=int, help="动作（0-添加后开始/1-添加后暂停）", location="form"
    )
    parser.add_argument("upload_limit", type=int, help="上传速度限制", location="form")
    parser.add_argument(
        "download_limit", type=int, help="下载速度限制", location="form"
    )
    parser.add_argument("ratio_limit", type=int, help="分享率限制", location="form")
    parser.add_argument(
        "seeding_time_limit", type=int, help="做种时间限制", location="form"
    )
    parser.add_argument(
        "downloader",
        type=str,
        help="下载器（Qbittorrent/Transmission/115网盘/Aria2）",
        location="form",
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="update_download_setting", data=self.parser.parse_args()
        )
...
class DownloadConfigDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_download_setting", data=self.parser.parse_args()
        )
...
class DownloadConfigDirectory(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_download_dirs", data=self.parser.parse_args()
        )
...
class UnknownDelete(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "id", type=str, help="未识别记录ID", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="del_unknown_path", data=self.parser.parse_args()
        )
...
class UnknownRename(ClientResource):
    ...
    parser.add_argument(
        "syncmod", type=str, help="转移模式", location="form", required=True
    )
...
class UnknownRenameUDF(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "inpath", type=str, help="源目录", location="form", required=True
    )
    parser.add_argument(
        "outpath", type=str, help="目的目录", location="form", required=True
    )
    parser.add_argument(
        "syncmod", type=str, help="转移模式", location="form", required=True
    )
...
class UnknownRedo(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "flag", type=str, help="类型（unknow/history）", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="re_identification", data=self.parser.parse_args()
        )
...
class TransferHistoryDelete(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "logids", type=list, help="记录IDS", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_history", data=self.parser.parse_args()
        )
...
class TransferHistoryList(ClientResource):
    ...
    parser.add_argument(
        "pagenum", type=int, help="每页条数", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_transfer_history", data=self.parser.parse_args()
        )
...
class SystemPath(ClientResource):
    ...
    parser.add_argument(
        "filter",
        type=str,
        help="过滤器（ONLYFILE/ONLYDIR/MEDIAFILE/SUBFILE/ALL）",
        location="form",
        required=True,
    )
...
class SystemMessage(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "lst_time", type=str, help="时间（YYYY-MM-DD HH24:MI:SS）", location="form"
    )
    ...
    def post(self):
        ...
        查询消息中心消息
        """
        return WebAction().get_system_message(
            lst_time=self.parser.parse_args().get("lst_time")
        )
...
class SystemProgress(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="refresh_process", data=self.parser.parse_args()
        )
...
class ConfigUpdate(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "items", type=dict, help="配置项", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="update_config", data=self.parser.parse_args().get("items")
        )
...
class ConfigTest(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "command", type=str, help="测试命令", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="test_connection", data=self.parser.parse_args()
        )
...
class ConfigRestore(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "file_name", type=str, help="备份文件名", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="restory_backup", data=self.parser.parse_args()
        )
...
class ConfigInfo(ClientResource):
    ...
    def post():
        ...
        获取所有配置信息
        """
        return {"code": 0, "success": True, "data": Config().get_config()}
...
class ConfigDirectory(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "oper", type=str, help="操作类型（add/sub/set）", location="form", required=True
    )
    parser.add_argument("key", type=str, help="配置项", location="form", required=True)
    parser.add_argument(
        "value", type=str, help="配置值", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="update_directory", data=self.parser.parse_args()
        )
...
class SubscribeDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="remove_rss_media", data=self.parser.parse_args()
        )
...
class SubscribeAdd(ClientResource):
    ...
    parser.add_argument("name", type=str, help="名称", location="form", required=True)
    parser.add_argument(
        "type", type=str, help="类型（MOV/TV）", location="form", required=True
    )
    parser.add_argument("year", type=str, help="发行年份", location="form")
    ...
    parser.add_argument(
        "fuzzy_match", type=int, help="模糊匹配（0-否/1-是）", location="form"
    )
    ...
    parser.add_argument(
        "over_edition", type=int, help="洗版（0-否/1-是）", location="form"
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="add_rss_media", data=self.parser.parse_args()
        )
...
class SubscribeMovieDate(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "id", type=str, help="TMDBID/DB:豆瓣ID", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="movie_calendar_data", data=self.parser.parse_args()
        )
...
class SubscribeTVDate(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="tv_calendar_data", data=self.parser.parse_args()
        )
...
class SubscribeSearch(ClientResource):
    ...
    parser.add_argument(
        "rssid", type=int, help="订阅ID", location="form", required=True
    )
...
class SubscribeInfo(ClientResource):
    ...
    parser.add_argument(
        "type", type=str, help="订阅类型（MOV/TV）", location="form", required=True
    )
...
class SubscribeRedo(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "rssid", type=int, help="订阅历史ID", location="form", required=True
    )
    parser.add_argument(
        "type", type=str, help="订阅类型（MOV/TV）", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="re_rss_history", data=self.parser.parse_args()
        )
...
class SubscribeHistoryDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_rss_history", data=self.parser.parse_args()
        )
...
class SubscribeHistory(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_rss_history", data=self.parser.parse_args()
        )
...
class RecommendList(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_recommend", data=self.parser.parse_args()
        )
...
class RssInfo(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_userrss_task", data=self.parser.parse_args()
        )
...
class RssDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_userrss_task", data=self.parser.parse_args()
        )
...
class RssUpdate(ClientResource):
    ...
    parser.add_argument(
        "name", type=str, help="任务名称", location="form", required=True
    )
    parser.add_argument(
        "address", type=str, help="RSS地址", location="form", required=True
    )
    parser.add_argument(
        "parser", type=int, help="解析器ID", location="form", required=True
    )
    parser.add_argument(
        "interval", type=int, help="刷新间隔（分钟）", location="form", required=True
    )
    parser.add_argument("uses", type=str, help="动作", location="form", required=True)
    parser.add_argument(
        "state", type=str, help="状态（Y/N）", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="update_userrss_task", data=self.parser.parse_args()
        )
...
class RssParserInfo(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_rssparser", data=self.parser.parse_args()
        )
...
class RssParserDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_rssparser", data=self.parser.parse_args()
        )
...
class RssParserUpdate(ClientResource):
    ...
    parser.add_argument("name", type=str, help="名称", location="form", required=True)
    parser.add_argument(
        "type", type=str, help="类型（JSON/XML）", location="form", required=True
    )
    parser.add_argument(
        "format", type=str, help="解析格式", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="update_rssparser", data=self.parser.parse_args()
        )
...
class RssParserList(ClientResource):
    ...
    def post():
        ...
        return {
            "code": 0,
            "success": True,
            "data": {"parsers": RssChecker().get_userrss_parser()},
        }
...
class RssPreview(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="list_rss_articles", data=self.parser.parse_args()
        )
...
class RssNameTest(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "taskid", type=int, help="任务ID", location="form", required=True
    )
    parser.add_argument("title", type=str, help="名称", location="form", required=True)
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="rss_article_test", data=self.parser.parse_args()
        )
...
class RssItemHistory(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="list_rss_history", data=self.parser.parse_args()
        )
...
class RssItemSet(ClientResource):
    ...
    parser.add_argument(
        "flag",
        type=str,
        help="操作类型（set_finished/set_unfinish）",
        location="form",
        required=True,
    )
    parser.add_argument(
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="rss_articles_check", data=self.parser.parse_args()
        )
...
class RssItemDownload(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="rss_articles_download", data=self.parser.parse_args()
        )
...
class MediaSearch(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "keyword", type=str, help="关键字", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="search_media_infos", data=self.parser.parse_args()
        )
...
class MediaCacheUpdate(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "key", type=str, help="缓存Key值", location="form", required=True
    )
    parser.add_argument("title", type=str, help="标题", location="form", required=True)
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="modify_tmdb_cache", data=self.parser.parse_args()
        )
...
class MediaCacheDelete(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "cache_key", type=str, help="缓存Key值", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_tmdb_cache", data=self.parser.parse_args()
        )
...
class MediaTvSeasons(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "tmdbid", type=str, help="TMDBID", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_tvseason_list", data=self.parser.parse_args()
        )
...
class MediaCategoryList(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_categories", data=self.parser.parse_args()
        )
...
class MediaSimilar(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="media_similar", data=self.parser.parse_args()
        )
...
class MediaRecommendations(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="media_recommendations", data=self.parser.parse_args()
        )
...
class MediaPersonList(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="person_medias", data=self.parser.parse_args()
        )
...
class MediaSubtitleDownload(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "path", type=str, help="文件路径（含文件名）", location="form", required=True
    )
    parser.add_argument(
        "name", type=str, help="名称（用于识别）", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="download_subtitle", data=self.parser.parse_args()
        )
...
class BrushTaskUpdate(ClientResource):
    ...
    parser.add_argument(
        "brushtask_name", type=str, help="任务名称", location="form", required=True
    )
    parser.add_argument(
        "brushtask_site", type=int, help="站点", location="form", required=True
    )
    parser.add_argument(
        "brushtask_interval",
        type=int,
    ...
    parser.add_argument(
        "brushtask_downloader", type=int, help="下载器", location="form", required=True
    )
    parser.add_argument(
        "brushtask_totalsize",
        type=int,
    ...
    parser.add_argument(
        "brushtask_state", type=str, help="状态（Y/N）", location="form", required=True
    )
    parser.add_argument(
        "brushtask_transfer", type=str, help="转移到媒体库（Y/N）", location="form"
    )
    parser.add_argument(
        "brushtask_sendmessage", type=str, help="消息推送（Y/N）", location="form"
    )
    parser.add_argument(
        "brushtask_forceupload", type=str, help="强制做种（Y/N）", location="form"
    )
    parser.add_argument(
        "brushtask_free", type=str, help="促销（FREE/2XFREE）", location="form"
    )
    parser.add_argument("brushtask_hr", type=str, help="Hit&Run（HR）", location="form")
    parser.add_argument(
        "brushtask_torrent_size", type=int, help="种子大小(GB)", location="form"
    )
    ...
    parser.add_argument(
        "brushtask_dlcount", type=int, help="同时下载任务数", location="form"
    )
    parser.add_argument(
        "brushtask_peercount", type=int, help="做种人数限制", location="form"
    )
    parser.add_argument(
        "brushtask_seedtime", type=float, help="做种时间(小时)", location="form"
    )
    parser.add_argument(
        "brushtask_seedratio", type=float, help="分享率", location="form"
    )
    parser.add_argument(
        "brushtask_seedsize", type=int, help="上传量(GB)", location="form"
    )
    parser.add_argument(
        "brushtask_dltime", type=float, help="下载耗时(小时)", location="form"
    )
    parser.add_argument(
        "brushtask_avg_upspeed", type=int, help="平均上传速度(KB/S)", location="form"
    )
    parser.add_argument(
        "brushtask_iatime", type=float, help="未活动时间(小时)", location="form"
    )
    parser.add_argument(
        "brushtask_pubdate", type=int, help="发布时间（小时）", location="form"
    )
    parser.add_argument(
        "brushtask_upspeed", type=int, help="上传限速（KB/S）", location="form"
    )
    parser.add_argument(
        "brushtask_downspeed", type=int, help="下载限速（KB/S）", location="form"
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="add_brushtask", data=self.parser.parse_args()
        )
...
class BrushTaskDelete(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "id", type=str, help="刷流任务ID", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="del_brushtask", data=self.parser.parse_args()
        )
...
class BrushTaskInfo(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="brushtask_detail", data=self.parser.parse_args()
        )
...
class BrushTaskList(ClientResource):
    ...
    def post():
        ...
        return {
            "code": 0,
            "success": True,
            "data": {"tasks": BrushTask().get_brushtask_info()},
        }
...
class BrushTaskTorrents(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="list_brushtask_torrents", data=self.parser.parse_args()
        )
...
class BrushTaskDownloaderUpdate(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "test", type=int, help="测试（0-否/1-是）", location="form", required=True
    )
    ...
    parser.add_argument(
        "type",
        type=str,
        help="类型（qbittorrent/transmission）",
        location="form",
        required=True,
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="add_downloader", data=self.parser.parse_args()
        )
...
class BrushTaskDownloaderDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_downloader", data=self.parser.parse_args()
        )
...
class BrushTaskDownloaderInfo(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_downloader", data=self.parser.parse_args()
        )
...
class BrushTaskDownloaderList(ClientResource):
    ...
    def post():
        ...
        return {
            "code": 0,
            "success": True,
            "data": {"downloaders": BrushTask().get_downloader_info()},
        }
...
class BrushTaskRun(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "id", type=int, help="刷流任务ID", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="run_brushtask", data=self.parser.parse_args()
        )
...
class FilterRuleGroupAdd(ClientResource):
    ...
    parser.add_argument("name", type=str, help="名称", location="form", required=True)
    parser.add_argument(
        "default", type=str, help="默认（Y/N）", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="add_filtergroup", data=self.parser.parse_args()
        )
...
class FilterRuleGroupRestore(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "groupids", type=list, help="规则组ID", location="form", required=True
    )
    parser.add_argument(
        "init_rulegroups", type=list, help="规则组脚本", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="restore_filtergroup", data=self.parser.parse_args()
        )
...
class FilterRuleGroupDefault(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="set_default_filtergroup", data=self.parser.parse_args()
        )
...
class FilterRuleGroupDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="del_filtergroup", data=self.parser.parse_args()
        )
...
class FilterRuleUpdate(ClientResource):
    ...
    parser.add_argument(
        "group_id", type=int, help="规则组ID", location="form", required=True
    )
    parser.add_argument(
        "rule_name", type=str, help="规则名称", location="form", required=True
    )
    parser.add_argument(
        "rule_pri", type=str, help="优先级", location="form", required=True
    )
    ...
    parser.add_argument(
        "rule_free", type=str, help="促销（FREE/2XFREE）", location="form"
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="add_filterrule", data=self.parser.parse_args()
        )
...
class FilterRuleDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="del_filterrule", data=self.parser.parse_args()
        )
...
class FilterRuleInfo(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "ruleid", type=int, help="规则ID", location="form", required=True
    )
    parser.add_argument(
        "groupid", type=int, help="规则组ID", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="filterrule_detail", data=self.parser.parse_args()
        )
...
class FilterRuleShare(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="share_filtergroup", data=self.parser.parse_args()
        )
...
class FilterRuleImport(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "content", type=str, help="规则内容", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="import_filtergroup", data=self.parser.parse_args()
        )
...
class WordsGroupAdd(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "tmdb_id", type=str, help="TMDBID", location="form", required=True
    )
    parser.add_argument(
        "tmdb_type", type=str, help="类型（movie/tv）", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="add_custom_word_group", data=self.parser.parse_args()
        )
...
class WordsGroupDelete(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "gid", type=int, help="识别词组ID", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_custom_word_group", data=self.parser.parse_args()
        )
...
class WordItemUpdate(ClientResource):
    ...
    parser.add_argument(
        "type",
        type=str,
        help="识别词类型（1-屏蔽/2-替换/3-替换+集偏移/4-集偏移）",
        location="form",
        required=True,
    )
    ...
    parser.add_argument(
        "regex", type=str, help="正则表达式（1-使用/0-不使用）", location="form"
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="add_or_edit_custom_word", data=self.parser.parse_args()
        )
...
class WordItemInfo(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "wid", type=int, help="识别词ID", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_custom_word", data=self.parser.parse_args()
        )
...
class WordItemDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_custom_word", data=self.parser.parse_args()
        )
...
class WordItemStatus(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "ids_info", type=list, help="识别词IDS", location="form", required=True
    )
    parser.add_argument(
        "flag", type=int, help="状态（1/0）", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="check_custom_words", data=self.parser.parse_args()
        )
...
class WordItemExport(ClientResource):
    ...
    parser.add_argument(
        "ids_info", type=str, help="识别词IDS（@_）", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="export_custom_words", data=self.parser.parse_args()
        )
...
class WordItemAnalyse(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "import_code", type=str, help="识别词代码", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="analyse_import_custom_words_code", data=self.parser.parse_args()
        )
...
class WordItemImport(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="import_custom_words", data=self.parser.parse_args()
        )
...
class SyncDirectoryUpdate(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="add_or_edit_sync_path", data=self.parser.parse_args()
        )
...
class SyncDirectoryInfo(ClientResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "sid", type=int, help="同步目录ID", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_sync_path", data=self.parser.parse_args()
        )
...
class SyncDirectoryDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_sync_path", data=self.parser.parse_args()
        )
...
class SyncDirectoryStatus(ClientResource):
    ...
    parser.add_argument(
        "flag", type=str, help="操作（rename/enable）", location="form", required=True
    )
    parser.add_argument(
        "checked", type=int, help="状态（0-否/1-是）", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="check_sync_path", data=self.parser.parse_args()
        )
...
class SyncDirectoryRun(ApiResource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "sid", type=int, help="同步目录ID", location="args", required=True
    )
    ...
    def get(self):
        ...
        """
        return WebAction().api_action(
            cmd="run_directory_sync", data=self.parser.parse_args()
        )
...
class MessageClientUpdate(ClientResource):
    ...
    parser.add_argument(
        "type",
        type=str,
        help="类型（wechat/telegram/serverchan/bark/pushplus/iyuu/slack/gotify）",
        location="form",
        required=True,
    )
    parser.add_argument(
        "config", type=str, help="配置项（JSON）", location="form", required=True
    )
    parser.add_argument(
        "switchs", type=list, help="开关", location="form", required=True
    )
    parser.add_argument(
        "interactive",
        type=int,
    ...
    parser.add_argument(
        "enabled", type=int, help="是否启用（0/1）", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="update_message_client", data=self.parser.parse_args()
        )
...
class MessageClientDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_message_client", data=self.parser.parse_args()
        )
...
class MessageClientStatus(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="check_message_client", data=self.parser.parse_args()
        )
...
class MessageClientInfo(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_message_client", data=self.parser.parse_args()
        )
...
class MessageClientTest(ClientResource):
    ...
    parser.add_argument(
        "config", type=str, help="配置（JSON）", location="form", required=True
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="test_message_client", data=self.parser.parse_args()
        )
...
class TorrentRemoverTaskInfo(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="get_torrent_remove_task", data=self.parser.parse_args()
        )
...
class TorrentRemoverTaskDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_torrent_remove_task", data=self.parser.parse_args()
        )
...
class TorrentRemoverTaskUpdate(ClientResource):
    ...
    parser.add_argument(
        "action",
        type=int,
        help="动作(1-暂停/2-删除种子/3-删除种子及文件)",
        location="form",
        required=True,
    )
    parser.add_argument(
        "interval", type=int, help="运行间隔（分钟）", location="form", required=True
    )
    parser.add_argument(
        "enabled",
        type=int,
    ...
    parser.add_argument(
        "onlynastool",
        type=int,
        help="只管理NASTool添加的下载（0-否/1-是）",
        location="form",
        required=True,
    )
    parser.add_argument("ratio", type=float, help="分享率", location="form")
    parser.add_argument(
        "seeding_time", type=int, help="做种时间（小时）", location="form"
    )
    parser.add_argument(
        "upload_avs", type=int, help="平均上传速度（KB/S）", location="form"
    )
    parser.add_argument("size", type=str, help="种子大小（GB）", location="form")
    parser.add_argument(
        "savepath_key", type=str, help="保存路径关键词", location="form"
    )
    ...
    parser.add_argument(
        "qb_state", type=str, help="Qb种子状态（多个用;分隔）", location="form"
    )
    parser.add_argument(
        "qb_category", type=str, help="Qb分类（多个用;分隔）", location="form"
    )
    parser.add_argument(
        "tr_state", type=str, help="Tr种子状态（多个用;分隔）", location="form"
    )
    parser.add_argument(
        "tr_error_key", type=str, help="Tr错误信息关键词", location="form"
    )
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="update_torrent_remove_task", data=self.parser.parse_args()
        )
...
class DoubanHistoryDelete(ClientResource):
    ...
    def post(self):
        ...
        """
        return WebAction().api_action(
            cmd="delete_douban_history", data=self.parser.parse_args()
        )
