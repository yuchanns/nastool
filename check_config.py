...
def check_config():
    ...
    if not Config().get_config("douban"):
        ...
        if (
            not Config().get_config("douban").get("users")
...
def update_config():
    ...
    if not _config.get("security"):
        _config["security"] = {
            "media_server_webhook_allow_ip": {"ipv4": "0.0.0.0/0", "ipv6": "::/0"},
            "telegram_webhook_allow_ip": {"ipv4": "127.0.0.1", "ipv6": "::/0"},
        }
        overwrite_cofig = True
    ...
    if not _config.get("scraper_nfo"):
        _config["scraper_nfo"] = {
            "movie": {"basic": True, "credits": True, "credits_chinese": False},
            "tv": {
                "basic": True,
                "credits": True,
    ...
    if not _config.get("downloaddir"):
        ...
        if dl_client and _config.get(dl_client):
            ...
            if not isinstance(container_path, dict):
                container_path = {
                    "movie": container_path,
                    "tv": container_path,
                    "anime": container_path,
                }
            downloaddir = []
        ...
        if msg_channel:
            ...
            if message.get("serverchan"):
                ...
                if sckey:
                    ...
                    interactive = 0
                    enabled = 1 if msg_channel == ctype else 0
                    client_config = json.dumps({"sckey": sckey})
                    _dbhelper.insert_message_client(
                        name=name,
                        ctype=ctype,
                        config=client_config,
            ...
            if message.get("bark"):
                ...
                if server and apikey:
                    ...
                    interactive = 0
                    enabled = 1 if msg_channel == ctype else 0
                    client_config = json.dumps({"server": server, "apikey": apikey})
                    _dbhelper.insert_message_client(
                        name=name,
                        ctype=ctype,
                        config=client_config,
            ...
            if message.get("iyuu"):
                ...
                if token:
                    ...
                    interactive = 0
                    enabled = 1 if msg_channel == ctype else 0
                    client_config = json.dumps({"token": token})
                    _dbhelper.insert_message_client(
                        name=name,
                        ctype=ctype,
                        config=client_config,
    ...
    try:
        ...
        for site in sites:
            ...
            # 站点过滤规则为|分隔的第2位
            rule_groupid = (
                str(site.NOTE).split("|")[1]
                if site.NOTE and len(str(site.NOTE).split("|")) > 1
                else ""
            )
            # 站点未读消息为|分隔的第3位
            site_unread_msg_notify = (
                str(site.NOTE).split("|")[2]
                if site.NOTE and len(str(site.NOTE).split("|")) > 2
                else "Y"
            )
            # 自定义UA为|分隔的第4位
            ua = (
                str(site.NOTE).split("|")[3]
                if site.NOTE and len(str(site.NOTE).split("|")) > 3
                else ""
            )
            # 是否开启浏览器仿真为|分隔的第5位
            chrome = (
                str(site.NOTE).split("|")[4]
                if site.NOTE and len(str(site.NOTE).split("|")) > 4
                else "N"
            )
            # 是否使用代理为|分隔的第6位
            proxy = (
                str(site.NOTE).split("|")[5]
                if site.NOTE and len(str(site.NOTE).split("|")) > 5
                else "N"
            )
            _dbhelper.update_config_site_note(
                tid=site.ID,
                note=json.dumps(
                    {
                        "parse": site_parse,
    ...
    try:
        ...
        for movie in rss_movies:
            ...
            _dbhelper.update_rss_movie_desc(
                rid=movie.ID, desc=json.dumps(__parse_rss_desc(movie.DESC))
            )
        # 电视剧订阅
